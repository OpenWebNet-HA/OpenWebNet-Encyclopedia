...
The request-repeat-suppress behavior is present in the `OPEN.db` `ScanAID` sequence and corroborated by observed traffic. The no-response termination rule is capture-derived behavior: the canonical frame database describes repetition but does not encode a declarative “empty pass” condition.
...
The final `#1` and `#0` forms select configured and unconfigured Devices respectively according to the implementation labels in `OPEN.db`. The precise Device-side definition of “configured” is not expanded by the source. It must not be assumed to mean that every Module is configured.
