...
This guide applies to the MyHOME Suite diagnostic-family workflow described under [Diagnostics](../diagnostics/). It is not a generic ZigBee discovery procedure. The ZigBee OpenWebNet interface instead exposes `WHO 1000 DIMENSION 81` neighbor discovery and separate [`WHO 13` scan/product-database operations](../functional/who-13-integration-gateway/zigbee-network-management.md#discovery-relationship-and-source-conflict); the available evidence does not establish Suite interview parity on that interface.
