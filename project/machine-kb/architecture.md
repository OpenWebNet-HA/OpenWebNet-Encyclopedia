...
The current `knowledge/` directories and privacy scanner are a skeleton. The [Phase 1 consumer contract](consumer-contract.md) now defines ID grammar, artifact encoding, and knowledge-state semantics; [schema versioning](schema-versioning.md) governs compatibility. Schema vocabulary, IR layout, and the initial release version are later gated decisions. See [Decisions](decisions.md) and [Roadmap](roadmap.md).
