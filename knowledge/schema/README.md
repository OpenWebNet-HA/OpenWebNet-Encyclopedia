...
[`../../build.py`](../../build.py) produces the pre-release `knowledge/manifest.json` from the shared IR and a fixed inventory of current public schema contracts plus the generated LLM corpus, retrieval chunks, and reference registries. The manifest lists schema paths, SHA-256 hashes, format versions, and applicable record counts. The manifest deliberately excludes its own hash. Its input digest covers the semantic IR and resolved reference records; it contains no timestamp, local path, random value, or consumer setting.
